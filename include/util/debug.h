...
#define RDMA_FILE_LOG(n, tid) \
  if (n >= RDMA_LOG_LEVEL)    \
  FileLogger((char*)__FILE__, __LINE__, n, tid).stream()

#define TLOG(n, tid)       \
  if (n >= RDMA_LOG_LEVEL) \
  ThreadLogger((char*)__FILE__, __LINE__, n, tid).stream()

// Do not know the actual thread ID
class FileLogger {
 public:
  FileLogger(const char* file, int line, int level, std::thread::id tid) : level_(level), tid_(tid) {
...
// Know the actual thread ID
class ThreadLogger {
 public:
  ThreadLogger(const char* file, int line, int level, t_id_t tid) : level_(level), tid_(tid) {
    if (level_ < RDMA_LOG_LEVEL)
      return;
    stream_ << "[" << StripBasename(std::string(file)) << ":" << line << "] ";
  }
